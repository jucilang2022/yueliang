...
function ProjectCard({ project }: { project: (typeof projects)[0] }) {
  ...
  return (
    <motion.div ref={ref} style={{ opacity }} className="relative h-[70vh] w-full flex items-center justify-center sticky top-0">
      <div className="relative w-full max-w-4xl mx-auto h-[500px] rounded-3xl overflow-hidden group border border-white/10 bg-gray-900">
        ...
        <div className="absolute inset-0 z-30 p-8 md:p-12 flex flex-col justify-end">
          <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            <h2 className="text-4xl md:text-6xl font-bold text-white mb-4">{project.title}</h2>
            <p className="text-lg text-white/80 max-w-md mb-6">{project.description}</p>
...
export function ProjectsView() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-32 pb-24 px-6 min-h-screen">
